...
**スコープ外:** 接続状態表示（オンライン/オフライン）は本フェーズでは実装しない
...
joinSession: async (code: string) => {
  ...
  try {
    // 1. joinCodeでセッション検索 → sessionIdを取得
    ...
    set({ session })
    // 4. localStorageにspeaker=Bを保存
    if (typeof window !== 'undefined') {
      localStorage.setItem(`speaker:${lookup.sessionId}`, 'B')
    }
  } catch (error) {
...
joinSession完了後、呼び出し側で `useSessionStore.getState().session?.id` から遷移先を取得。
...
startPolling: () => {
  ...
  const interval = setInterval(async () => {
    try {
      ...
      // updatedAtが変わったら全件取得（メッセージ、ステータス、参加状態すべてカバー）
...
**Step 3: speaker管理ヘルパー**

`src/lib/speaker-storage.ts`:
```typescript
...
export function getSpeakerForSession(sessionId: string): Speaker {
  if (typeof window === 'undefined') return 'A' // SSRフォールバック
  const stored = localStorage.getItem(`speaker:${sessionId}`)
  if (stored === 'A' || stored === 'B') return stored
  return 'A' // フォールバック（作成者=A）
}
```

- Aさん: createSession成功後に `setSpeakerForSession(id, 'A')`
- Bさん: joinSession成功後に `setSpeakerForSession(id, 'B')`
- 対話画面: マウント時に `getSpeakerForSession(id)` で自分のspeakerを取得

**Step 4: addMessageのspeaker固定**

multiモード時、ストアのaddMessageで `getSpeakerForSession(session.id)` を使う（currentSpeakerではなく）。singleモードは既存のcurrentSpeaker動作のまま。
